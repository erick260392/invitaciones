...
<div class="min-h-screen bg-rose-50 p-4">
    <div class="max-w-md mx-auto bg-white rounded-lg shadow-lg p-6">
        <!-- Encabezado -->
        <img src="{{ asset($invitation->cover_image ?? 'default-cover.jpg') }}" class="w-full h-48 object-cover rounded-lg mb-4">
        ...
        <div class="text-center space-y-2">
            ...
            <p class="text-gray-600">
                📅 {{ $invitation->event_date->isoFormat('dddd D [de] MMMM [de] YYYY') }}<br>
                🕒 {{ $invitation->event_date->format('H:i') }} horas
            </p>
            <p class="text-gray-800 mt-4">📍 {{ $invitation->location }}</p>
            <a href="https://maps.google.com/?q={{ urlencode($invitation->location) }}"
               class="inline-block mt-2 text-rose-600 hover:text-rose-700"
               target="_blank">
                Ver en mapa
            </a>
        </div>
